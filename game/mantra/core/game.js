(function() {
  ...
  Mantra.Game = (function() {
    ...
    function Game(options) {
      ...
      if (this.options.center_coordinates) this.center_coordinates = true;
      this.state = new FSM('initialized', {
        name: 'initialized'
      });
      this.state.add_transition('start', 'initialized', null, 'started');
      this.state.add_transition_from_any_state('lose', (function() {
        return _this.options.process_game_over.call(_this);
      }), 'game_lost');
      ...
      if (this.options.on_keypress) this.key_map = this.options.on_keypress;
      if (this.options.assets) this.assets(this.options.assets);
    }
    ...
    Game.prototype.update = function() {
      ...
      if (this.entities.length) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          entity = _ref[_i];
          if (!entity.remove_from_world) entity.update();
        }
        _ref2 = this.entities;
    ...
    Game.prototype.draw = function(callback) {
      ...
      this.context.save();
      if (this.center_coordinates) this.translateToCenter();
      _ref = this.entities;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        entity = _ref[_i];
    ...
    Game.prototype.startGameLoop = function(options) {
      ...
      if (this.screens[options.on_screen]) this.showScreen(options.on_screen);
    ...
    Game.prototype.onKey = function(key) {
      if (this.key_map[key]) this.key_map[key]();
      if (this.currentScreen) return this.currentScreen.onKey(key);
    };

    Game.prototype.showScreen = function(screen) {
      var name, skreen, _ref;
      if (typeof screen === 'string') screen = this.screens[screen];
    ...
    Game.prototype.startInput = function() {
      ...
      getXandY = function(e) {
        var x, y;
        x = e.clientX - _this.canvas.getBoundingClientRect().left;
        if (_this.center_coordinates) x -= _this.canvas.width / 2;
        y = e.clientY - _this.canvas.getBoundingClientRect().top;
        if (_this.center_coordinates) y -= _this.canvas.height / 2;
        return {
          x: x,
          y: y
        };
